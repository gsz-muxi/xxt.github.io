...
def init_chaoxing(common_config, tiku_config):
    ...
    if not username or not password:
        ...
        username = "18595730995"
        password = "15083397316Cc"
...
def filter_courses(all_course, course_list):
    ...
    while True:  # 循环直到成功选择至少一门课程
        if not course_list:
            ...
            print("*" * 28)
            try:
                course_list_input = input(
                    "请输入想要学习的课程列表,以逗号分隔,例: 2151141,189191,198198\n"
                ).split(",")
                course_list = [id.strip() for id in course_list_input if id.strip()]
        ...
        # 如果没有找到指定课程，提示用户重新输入
        if not course_task:
            print("未找到指定的课程，请重新输入！")
            course_list = []  # 重置course_list，重新提示输入
            continue
